...
@CommandPkg(
		...
		name = "CredToString", label = "CredToString",
		node_label = "CredToString", description = "Converts Credential to String", icon = "pkg.svg",
		
		//Return type information. return_type ensures only the right kind of variable is provided on the UI. 
		return_label = "[[CredToString.return_label]]", return_type = STRING, return_required = true)
public class CredToString {
	...
	public Value<String> action(
			...
			//UI labels.
			@Pkg(label = "Converts a credential variable to String")
			//Ensure that a validation error is thrown when the value is null.
			@NotEmpty
			SecureString firstString) {
		
		//Internal validation, to disallow empty strings. No null check needed as we have NotEmpty on firstString.
		if ("".equals(firstString.getInsecureString().trim()))
			throw new BotCommandException(MESSAGES.getString("emptyInputString", "firstString"));


		//Business logic
		String result = firstString.getInsecureString();
